...
class Tugas {
	...
	//---------------------------------------------------------------------------------
	//---	 Fungsi untuk Menghapus Data dengan Metode Linear Search (Loop: For)	---
	//---------------------------------------------------------------------------------
	public static void cariDataLinear(formatBiodata biodataMahasiswa[])
	{
		...
		System.out.println("---------------------------------------------------------------");
		for(int k = 0; k <= N-1; k++){
			tampilkanData(biodataMahasiswa, k);
		}
		System.out.println("---------------------------------------------------------------");
		Scanner masukan = new Scanner(System.in);
		System.out.print("\nMasukkan nama yang ingin dihapus datanya : ");
		String dataHapus = masukan.next();
		for (int i=0; i <= N-1; i++){
			if (biodataMahasiswa[i].nama.equalsIgnoreCase(dataHapus)){
				hasil = true;
				hapusDataTengah(biodataMahasiswa, i, dataHapus);
				System.out.println("\n-------------------------------------------------------------");
				System.out.println("NAMA\tALAMAT\tUMUR\tJEKEL\tHOBI[0]\tHOBI[1]\tHOBI[2]\tIPK");
				System.out.println("-------------------------------------------------------------");
				for(int k = 0; k <= N-1; k++){
			...
			System.out.println("\nMaaf, data dari nama yang dimasukan tidak ditemukan.");
			System.out.println("\n-------------------------------------------------------------");
			System.out.println("NAMA\tALAMAT\tUMUR\tJEKEL\tHOBI[0]\tHOBI[1]\tHOBI[2]\tIPK");
			System.out.println("-------------------------------------------------------------");
			for(int k = 0; k <= N-1; k++){
	...
	public static void hapusDataTengah(formatBiodata biodataMahasiswa[], int y, String dataNama){
		for (int i=y; i<= N-2; i++){
			biodataMahasiswa[i] = biodataMahasiswa[i+1];
		}
		System.out.println("Data ditemukan pada larik ke-"+y+", semua data " + dataNama + " telah dihapus.");
		N--;
	}
	//-------------------------------------------------
