...
class hashingLatihan {
	...
	public static void tampilkanData(formatBiodata biodataMahasiswa[]) {
		...
		for (int i=0; i<=N-1; i++) {
			...
			if (i % 100 == 0) berhentiSebentar();
		}
		System.out.println("-------------------------------------------------------");
	}
	public static void searchNIMHash(formatBiodata biodataMahasiswa[]) {
		Scanner input = new Scanner(System.in);
		System.out.print("Masukkan NIM yang ingin anda cari : ");
		int searchNim = input.nextInt();
		int hitungHashingNim = searchNim % 997;
		for (int i=0; i<=N-1; i++) {
			int hashingDataNim = hitungNilaiHash(biodataMahasiswa[i].nim);
			if(hitungHashingNim == hashingDataNim) {
				System.out.println("Data dengan Nim "+searchNim+" ditemukan pada baris ke "+i);
				break;
			}
		}
	}
	public static void main(String[] args) {
		...
		searchNIMHash(biodataMahasiswa);
	}
}